...
func CactusSignature() *big.Int {
	return new(big.Int).Set(cactusSignature)
}

func (Cactus) GetSignature() *big.Int {
	return CactusSignature()
}

func (e Cactus) Insert(r column.Row) error {
	r.SetInt("ENTITY_TYPE", int(pb.Entity_CACTUS))
	r.SetFloat64("X", e.X)
	r.SetFloat64("Y", e.Y)
